"""
DIST (DiT-based) 基因表达超分辨
基于 DiT 的空间转录组数据超分辨率增强
"""
...
def get_not_in_tissue_coords(coords, img_xy):
    ...
    return not_in_tissue_coords, np.array(not_in_tissue_index).astype(int)
...
def getSTtrainset(train_counts, train_coords):
    ...
    lr_spot_index = []
    lr_x, lr_y = np.mgrid[
        0 : max(train_coords[:, 0]) : 2 * delta_x,
    ...
    for i in range(train_counts.shape[1]):
        # griddata(point, value, xi(object), method)
    ...
    hr_x, hr_y = np.mgrid[
        0 : max(train_coords[:, 0]) + delta_x : delta_x,
...
def get_ST_position_info(integral_coords):
    ...
    integral_coords = integral_coords.astype(float)
    imputed_barcodes=[str(val[0])+"x"+str(val[1]) for val in np.vstack((imputed_x.reshape(-1),imputed_y.reshape(-1))).T]
    imputed_coords = pd.DataFrame(np.vstack((imputed_x.reshape(-1),imputed_y.reshape(-1))).astype(float).T,
                               columns=['row','col'],index=imputed_barcodes)    
    neighbor_matrix=pd.DataFrame(np.zeros((imputed_coords.shape[0],imputed_coords.shape[0]),dtype=int),
...
def img2expr(imputed_img, gene_ids, integral_coords, position_info):
    ...
    integral_coords = [list(i.astype(float)) for i in np.array(integral_coords)]
...
def DIST(
    ...
    **kwargs,
    ...
    """
    在 train_set 上训练 DIST 模型，在 test_set 上测试，得到插值后的基因表达。

    Args:
        train_set: 训练集，来自 getSTtrainset()
        test_set: 测试集，来自 getSTtestset()
        epoch: 训练轮数，默认 200
        batch_size: 批大小，默认 128
        learning_rate: 学习率，默认 0.001
        gpu: 指定 GPU，如 '0' 或 '0,2,3'，对应 os.environ["CUDA_VISIBLE_DEVICES"]
        **kwargs: 其他 DiT 参数，如 hidden_dim, num_heads, num_layers, dropout 等

    Returns:
        imputed_img: 3-D ndarray，形状为 (基因数, 行, 列)，插值后的基因表达
    """
    if gpu is not None:
    ...
    net = network_dit.Net(train_set, test_set, conf)
    return net.run()